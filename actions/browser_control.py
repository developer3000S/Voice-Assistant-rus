...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".Anfisa_profiles" / browser
...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".Anfisa_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using Anfisa profile")
                Anfisa = str(Path.home() / ".Anfisa_profiles" / "firefox_Anfisa")
                Path(Anfisa).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(Anfisa, **kwargs)
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".Anfisa_profiles" / "safari")
        ...
        # automation). Fall back to a persistent Anfisa automation profile —
        # accounts logged in here once stay logged in on later sessions too.
        Anfisa_profile = str(Path.home() / ".Anfisa_profiles" / self.browser_name)
        Path(Anfisa_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with Anfisa profile: {Anfisa_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(Anfisa_profile, **kwargs)
            self._page = await self._adopt_page()
            print(f"[Browser] ✅ Launched [{label}] with Anfisa profile "
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "Anfisa_screenshot.png")
