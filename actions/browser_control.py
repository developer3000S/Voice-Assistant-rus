...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".Анфиса_profiles" / browser
...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".Анфиса_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using Анфиса profile")
                Анфиса = str(Path.home() / ".Анфиса_profiles" / "firefox_Анфиса")
                Path(Анфиса).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(Анфиса, **kwargs)
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".Анфиса_profiles" / "safari")
        ...
        # automation). Fall back to a persistent Анфиса automation profile —
        # accounts logged in here once stay logged in on later sessions too.
        Анфиса_profile = str(Path.home() / ".Анфиса_profiles" / self.browser_name)
        Path(Анфиса_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with Анфиса profile: {Анфиса_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(Анфиса_profile, **kwargs)
            self._page = await self._adopt_page()
            print(f"[Browser] ✅ Launched [{label}] with Анфиса profile "
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "Анфиса_screenshot.png")
    ...
    def list_sessions(self) -> str:
        with self._lock:
            ...
            for name in self._sessions:
                Анфисаer = " ◀ active" if name == self._active_browser else ""
                lines.append(f"  • {name}{Анфисаer}")
