...
try:
    ...
    notification.notify(title="ANFISA Reminder", message=message, timeout=15)
    notified = True
except Exception:
    pass
...
if not notified:
    try:
        ...
        ToastNotifier().show_toast("ANFISA Reminder", message, duration=15, threaded=False)
        notified = True
    except Exception:
        pass
...
if not notified:
    try:
        import subprocess
        script = 'display notification "{{}}" with title "Anfisa Reminder"'.format(
            message.replace('"', '')
        )
...
"""
    ...
    script_body = f"""# Сгенерировано напоминанием Anfisa — не редактируйте
import sys, os, pathlib
{notify_block}
# Самоудаление после срабатывания
try:
...
"""
    ...
    script_path.chmod(0o600)   # чтение и запись только для владельца
    return script_path
...
def _schedule_windows(target_dt: datetime, task_name: str,
    ...
    xml_content = (
        ...
        '  <RegistrationInfo><Description>Anfisa Reminder</Description></RegistrationInfo>\n'
...
def _schedule_mac(target_dt: datetime, task_name: str,
    ...
    label     = f"com.Anfisa.reminder.{task_name}"
...
def _schedule_linux(target_dt: datetime, task_name: str,
    ...
    if shutil.which("systemd-run"):
        ...
        if result.returncode == 0:
            return task_name
        print(f"[Reminder] ⚠️ systemd-run не сработал: {result.stderr.strip()}, пробую 'at'")
    ...
    print("[Reminder] ❌ На этой системе Linux не найдено ни systemd-run, ни at.")
    return ""
...
def reminder(
    ...
    message  = parameters.get("message", "Напоминание").strip()

    if not date_str or not time_str:
        return "Сэр, чтобы поставить напоминание, мне нужны и дата, и время."

    try:
        target_dt = datetime.strptime(f"{date_str} {time_str}", "%Y-%m-%d %H:%M")
    except ValueError:
        return "Сэр, я не смогла разобрать эту дату или время. Используйте формат YYYY-MM-DD и HH:MM."

    if target_dt <= datetime.now():
        return "Сэр, это время уже прошло — я не могу поставить напоминание в прошлом."

    os_name    = _get_os()
    safe_msg   = _sanitise(message)
    task_name  = f"AnfisaReminder_{target_dt.strftime('%Y%m%d_%H%M%S')}"

    try:
        script_path = _write_notify_script(task_name, safe_msg, os_name)
    except Exception as e:
        return f"Сэр, не удалось подготовить скрипт напоминания: {e}"
    ...
    try:
        ...
        script_path.unlink(missing_ok=True)
        print(f"[Reminder] ❌ Исключение при планировании: {e}")
        return "Сэр, при постановке напоминания что-то пошло не так."

    if not job_id:
        return "Сэр, мне не удалось зарегистрировать напоминание в системном планировщике."

    if player:
        player.write_log(f"[Reminder] ✅ {date_str} {time_str} — {safe_msg[:40]}")

    friendly_time = target_dt.strftime("%B %d в %I:%M %p")
    return f"Сэр, напоминание установлено на {friendly_time}."


# ── Описание инструмента (авто-обнаружение через core/action_loader.py) ──────
TOOL = {
    "name": "reminder",
    "description": "Ставит напоминание на заданное время через планировщик заданий.",
    "parameters": {
        "type": "OBJECT",
        "properties": {
            "date": {
            ...
            "time": {
                "type": "STRING",
                "description": "Время в формате HH:MM (24-часовой)"
            },
            "message": {
                "type": "STRING",
                "description": "Текст сообщения напоминания"
            }
        },
        "required": [
            "date",
