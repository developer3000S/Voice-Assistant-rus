...
| Функция | Описание |
|---|---|
| 🎙️ Слово пробуждения | Локальное обнаружение **«Привет Anfisa»** — спит, пока не позовут; сама засыпает через 2 мин тишины; пока спит — аудио в облако не уходит. Опционально, загрузка одним кликом, переключение и ручной сон/пробуждение из интерфейса |
...
| **Микрофон** | Нужен для голосового общения (и для слова пробуждения «Привет Anfisa») |
...
│   └── wake_word.py          | Локальный детектор «Привет Anfisa» — своя нить, оффлайн, опционально
└── config/
